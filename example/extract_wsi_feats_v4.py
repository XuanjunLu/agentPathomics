...
def extract_patch_feats(image_path, nuclei_instance_path, nuclei_class_path, tumor_path, save_dir, feature_type=None,
                        nuclei_type=None, identifier=None) -> None:
    """
    ...
    :param identifier: str, "nuclei", "mitosis", identifier for feature extraction in nuclei
    ...
    image_feats_save_path = save_dir / identifier / "image_level" / wsi_id / f"{name}.csv"
    nuclei_feats_save_path = save_dir / identifier / "nuclei_level" / wsi_id / f"{name}.csv"
    ...
    df_image = df_image.add_suffix(f"_{identifier}")
    ...
    df_nuclei = df_nuclei.add_suffix(f"_{identifier}")
    ...
    for j in range(len(df_nuclei)):
        centroid_x = df_nuclei.loc[j, f'Basic_CentroidX_{identifier}']
        centroid_y = df_nuclei.loc[j, f'Basic_CentroidY_{identifier}']
...
def tissue_extract_patch_feats(image_path, roi_path, save_dir, identifier=None):
    extractor = featureextractor.PathomicsFeatureExtractor()
    extractor.enableFeaturesByName(
        # image_levels
        firstorder=[],
...
def read_mask(mask_path, width, height):
...
def extract_patches(wsi_path, nuclei_instance_path, nuclei_class_path, tumor_path, save_dir, mag=40,
                    patch_size=1024, n_workers=32, identifier=None):
    wsi_obj = get_file_handler(wsi_path)
    wsi_name = wsi_path.stem
    width, height = wsi_obj.get_dimensions(read_mag=mag)
    tumor_mask = read_mask(tumor_path, width, height)
    nuclei_instance_mask = read_mask(nuclei_instance_path, width, height)
    nuclei_class_mask = read_mask(nuclei_class_path, width, height)
    gray_level = len(np.unique(nuclei_class_mask))
    if gray_level >= 20:
        raise ValueError(f"The grayscale level in {nuclei_class_path} exceeds the limit. "
                         ...
                         f"5 for non-neoplastic cells and so on")
...
def tissue_extract_patches(wsi_path, roi_path, save_dir, mag=40, patch_size=1024, n_workers=32, identifier=None):
    wsi_obj = get_file_handler(wsi_path)
    wsi_name = wsi_path.stem
    width, height = wsi_obj.get_dimensions(read_mag=mag)
    roi_mask = read_mask(roi_path, width, height)
    coords = gen_coords(width, height, patch_size, patch_size)
    # only select coords within nuclei and tumor masks
    coords = [
        coord for coord in coords if check_coords_in_mask(coord, patch_size, (width, height),
...
def nuclei_feats_extraction(wsi_dir: Path, nuclei_dir: Path, tumor_dir: Path, save_dir: Path, mask_suffix="png", wsi_num=None,
                            mag=40, n_workers=25, feature_type=None, nuclei_type=None, identifier=None):
    """
    ...
    image_patch_dir = save_dir / identifier / "patches" / "image"
    nuclei_instance_patch_dir = save_dir / identifier / "patches" / "nuclei_instance"
    nuclei_class_patch_dir = save_dir / identifier / "patches" / "nuclei_class"
    tumor_patch_dir = save_dir / identifier / "patches" / "tumor"

    for wsi_path in wsi_paths:
        wsi_name = wsi_path.stem
        print(f'=====Extracting {identifier} feats in {wsi_name}=====')
        ...
        image_feats_save_path = save_dir / identifier / "wsi_image_level" / f"{wsi_name}.csv"
        nuclei_feats_save_path = save_dir / identifier / "wsi_nuclei_level" / f"{wsi_name}.csv"
        ...
        print(f'==Processing {identifier} patches: {len(image_patches)}==')

        # pbar = tqdm(total=len(image_patches), desc=f'Extracting feats in {wsi_name}')
        # def update_bar(*a):
        #     pbar.update()
        # pool = Pool(n_workers)
        ...
        iteration = []
        for image_patch, nuclei_instance_patch, nuclei_class_patch, tumor_patch in zip(image_patches, nuclei_instance_patches, nuclei_class_patches, tumor_patches):
            iteration.append((image_patch, nuclei_instance_patch, nuclei_class_patch, tumor_patch, save_dir, feature_type, nuclei_type, identifier))
        with Pool(n_workers) as pool:
            for _ in pool.starmap(extract_patch_feats, iteration):
                break
                pass
            pool.close()
            pool.join()
        ...
        # merge
        image_feats_folder = save_dir / identifier / "image_level" / wsi_name
        nuclei_feats_folder = save_dir / identifier / "nuclei_level" / wsi_name
...
def tissue_feats_extraction(wsi_dir: Path, roi_dir: Path, save_dir: Path, mask_suffix="png", wsi_num=200, mag=40,
                            n_workers=25):
    """
    ...
    :param n_workers: integer, the number of processes
    :param identifier: str, "tls", "tum", "str", "lym", "nec", identifier for feature extraction in roi

    :return: csv file, feature matrix, each column represent a feature name, each row represents a patch
    """

    identifier = os.path.basename(roi_dir)
    wsi_paths = list(wsi_dir.glob("*.*"))
    # extracted_list_path = '/mnt/hd0/project/bca21gene/generated/fahzu/extracted_names.csv'
    # extracted_names = pd.read_csv(extracted_list_path)['name'].values
    ...
    for wsi_path in tqdm(wsi_paths, desc='Extracting Patches'):
        ...
        tissue_extract_patches(wsi_path, roi_path, save_dir, mag=mag, n_workers=n_workers, identifier=identifier)

    image_patch_dir = save_dir / identifier / 'patches' / 'image'
    roi_patch_dir = save_dir / identifier / 'patches' / 'roi'
    ...
    for wsi_path in wsi_paths:
        ...
        #     pbar.update()
        # pool = Pool(n_workers)
        # for image_patch, roi_patch in zip(image_patches, roi_patches):
        #     # print(image_patch.stem, nuclei_instance_patch.stem, nuclei_class_patch.stem, tumor_patch.stem)
        #     pool.apply_async(
        #         tissue_extract_patch_feats,
        #         args=(image_patch, roi_patch, save_dir, identifier),
        #         callback=update_bar
        #     )
        # pool.close()
        # pool.join()
        ...
        iteration = []
        for image_patch, roi_patch in zip(image_patches, roi_patches):
            iteration.append((image_patch, roi_patch, save_dir, identifier))
        with Pool(n_workers) as pool:
            for _ in pool.starmap(tissue_extract_patch_feats, iteration):
                break
                pass
            pool.close()
            pool.join()
...
def main():
    parser = argparse.ArgumentParser(description="Nuclei and tissue feature extraction")
    parser.add_argument("--wsi_dir", default="../example_folder/svs", type=Path, required=True, help="WSIs folder")
    parser.add_argument("--nuclei_dir", default="../example_folder/nuclei_seg", type=Path, required=True, help="Nuclei segmentation folder")
    parser.add_argument("--tumor_dir", default="../example_folder/tissue_seg/tumor", type=Path, required=True, help="Tumor mask folder")
    parser.add_argument("--nuclei_save_dir", default="../example_folder/nuclei_feats_extraction", type=Path, required=True, help="Nuclei storage folder")
    parser.add_argument("--tissue_roi_dir", default="../example_folder/tissue_seg", type=Path, required=True, help="Multi-class tissue mask folder")
    parser.add_argument("--tissue_save_dir", default="../example_folder/tissue_feats_extraction", type=Path, required=True, help="Tissue storage folder")
    args = parser.parse_args()

    #=============== Nuclei feats extraction ===============
    nuclei_feats_extraction(args.wsi_dir, args.nuclei_dir, args.tumor_dir, args.nuclei_save_dir,
                            mask_suffix="png", wsi_num=2, mag=40, n_workers=25, feature_type=None,
                            nuclei_type=None, identifier="nuclei")

    #=============== Tissue feats extraction ===============
    for tissue in os.listdir(args.tissue_roi_dir):
        if tissue != "full_images":
            roi_dir = args.tissue_roi_dir / tissue
            tissue_feats_extraction(args.wsi_dir, roi_dir, args.tissue_save_dir, mask_suffix="png", wsi_num=2, mag=40, n_workers=25)
