...
if __name__ == '__main__':
    ...
    mask_files = [img_file.replace(image_suffix, mask_suffix) for img_file in img_files] 
    select_size = 300
    ...
    with Pool(processes=n_workers) as pool:
        featureVectors = pool.starmap(
            run, zip(img_files, mask_files, repeat(features, len(img_files))))

    # Step 6: save results
    save_results_csv_path = f'{save_dir}/{patient_id}_10x_stroma_feat.csv'
    data = {}
    data['patient_id'] = []
    data['patch_id'] = []
