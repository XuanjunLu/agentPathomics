...
def main():
    ...
    save_dir = Path("/mnt/hd0/project/bca21gene/generated/fahzu/feats2")
    ...
    for wsi_path in tqdm(wsi_paths, desc='Extracting Patches'):
        wsi_name = wsi_path.stem
        if wsi_name in extracted_names:
            continue
    ...
    image_patches = list(image_patch_dir.rglob("*.*"))
    nuclei_instance_patches = list(nuclei_instance_patch_dir.rglob("*.*"))
    nuclei_class_patches = list(nuclei_class_patch_dir.rglob("*.*"))
    tumor_patches = list(tumor_patch_dir.rglob("*.*"))

    # extract image and nuclei level feats from patches
    pbar = tqdm(total=len(image_patches), desc='Extracting Features')
    def update_bar(*a):
        pbar.update()
    pool = Pool(n_workers)
    ...
    # merge image and nuclei level feats
