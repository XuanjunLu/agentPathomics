...
class PathomicsGLRLM(base.PathomicsFeaturesBase):
    r"""
  ...
  """
    ...
    def __init__(self, inputImage, inputMask, **kwargs):
        super(PathomicsGLRLM, self).__init__(inputImage, inputMask, **kwargs)
        self.name = 'GLRLM'
    ...
    def _calculateMatrix(self, voxelCoordinates=None):
        ...
        for _i in range(len(self.roi_masks)):
            ...
            matrix_args = [
                self.imageArray, maskArray, Ng, Nr,
            ...
            # P_glrlm, _ = cMatrices.calculate_glrlm(
            #     *matrix_args)
            ...
            Nrs.append(Nr)       
            glrlms.append(P_glrlm)

        self.coefficients['Nr'] = Nrs
        return glrlms
    ...
    def _calculateCoefficients(self):
        ...
        ivectors = []
        jvectors = []
        for _i in range(len(self.P_glrlm)):
          pr = numpy.sum(self.P_glrlm[_i], 1)  # shape (Nvox, Nr, Na)
          pg = numpy.sum(self.P_glrlm[_i], 2)  # shape (Nvox, Ng, Na)

          ivector = self.coefficients['grayLevels'].astype(float)  # shape (Ng,)
          jvector = numpy.arange(1,
                                self.P_glrlm[_i].shape[2] + 1,
                                dtype=numpy.float64)  # shape (Nr,)

          # Delete columns that run lengths not present in the ROI
          emptyRunLenghts = numpy.where(numpy.sum(pr, (0, 2)) == 0)
          self.P_glrlm[_i] = numpy.delete(self.P_glrlm[_i], emptyRunLenghts, 2)
    ...
    **1. Short Run Emphasis (SRE)**
    ...
    """   
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          sre = numpy.sum((pr[_i] / (jvector[_i][None, :, None]**2)), 1) / Nr[_i]
          res.append(numpy.nanmean(sre, 1))
    ...
    **2. Long Run Emphasis (LRE)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          lre = numpy.sum((pr[_i] * (jvector[_i][None, :, None]**2)), 1) / Nr[_i]
          res.append(numpy.nanmean(lre, 1))
        return numpy.array(res).astype('float')
    ...
    **3. Gray Level Non-Uniformity (GLN)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          gln = numpy.sum((pg[_i]**2), 1) / Nr[_i]
          res.append(numpy.nanmean(gln, 1))
        return numpy.array(res).astype('float')
    ...
    **4. Gray Level Non-Uniformity Normalized (GLNN)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          glnn = numpy.sum(pg[_i]**2, 1) / (Nr[_i]**2)
          res.append(numpy.nanmean(glnn, 1))
        return numpy.array(res).astype('float')
    ...
    **5. Run Length Non-Uniformity (RLN)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          rln = numpy.sum((pr[_i]**2), 1) / Nr[_i]
          res.append(numpy.nanmean(rln, 1))
        return numpy.array(res).astype('float')
    ...
    **6. Run Length Non-Uniformity Normalized (RLNN)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          rlnn = numpy.sum((pr[_i]**2), 1) / Nr[_i]**2
          res.append(numpy.nanmean(rlnn, 1))
        return numpy.array(res).astype('float')
    ...
    **7. Run Percentage (RP)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          Np = numpy.sum(pr[_i] * jvector[_i][None, :, None], 1)
          rp = Nr[_i] / Np
          res.append(numpy.nanmean(rp, 1))
        return numpy.array(res).astype('float')
    ...
    **8. Gray Level Variance (GLV)**
    ...
    """
        ...
        # pg = self.coefficients[
        #     'pg'] / Nr[:, None, :]  # divide by Nr to get the normalized matrix
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          pg = self.coefficients['pg'][_i] / Nr[_i][:, None, :]
          u_i = numpy.sum(pg * ivector[_i][None, :, None], 1, keepdims=True)
          glv = numpy.sum(pg * (ivector[_i][None, :, None] - u_i)**2, 1)
          res.append(numpy.nanmean(glv, 1))
        return numpy.array(res).astype('float')
    ...
    **9. Run Variance (RV)**
    ...
    """
        ...
        # pr = self.coefficients[
        #     'pr'] / Nr[:, None, :]  # divide by Nr to get the normalized matrix
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          pr = self.coefficients['pr'][_i] / Nr[_i][:, None, :]
          u_j = numpy.sum(pr * jvector[_i][None, :, None], 1, keepdims=True)
          rv = numpy.sum(pr * (jvector[_i][None, :, None] - u_j)**2, 1)
          res.append(numpy.nanmean(rv, 1))
        return numpy.array(res).astype('float')
    ...
    **10. Run Entropy (RE)**
    ...
    """
        eps = numpy.spacing(1)
        Nr = self.coefficients['Nr']
        # p_glrlm = self.P_glrlm / Nr[:, None,
        #                             None, :]  # divide by Nr to get the normalized matrix

        # re = -numpy.sum(p_glrlm * numpy.log2(p_glrlm + eps), (1, 2))
        # return numpy.nanmean(re, 1)
        res = []
        for _i in range(len(self.P_glrlm)):
          p_glrlm = self.P_glrlm[_i] / Nr[_i][:, None, None, :]
          re = -numpy.sum(p_glrlm * numpy.log2(p_glrlm + eps), (1, 2))
          res.append(numpy.nanmean(re, 1))
        return numpy.array(res).astype('float')
    ...
    **11. Low Gray Level Run Emphasis (LGLRE)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          lglre = numpy.sum((pg[_i] / (ivector[_i][None, :, None]**2)), 1) / Nr[_i]
          res.append(numpy.nanmean(lglre, 1))
        return numpy.array(res).astype('float')
    ...
    **12. High Gray Level Run Emphasis (HGLRE)**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glrlm)):
          hglre = numpy.sum((pg[_i] * (ivector[_i][None, :, None]**2)), 1) / Nr[_i]
          res.append(numpy.nanmean(hglre, 1))
        return numpy.array(res).astype('float')
    ...
    **13. Short Run Low Gray Level Emphasis (SRLGLE)**
    ...
    """
        ...
        # srlgle = numpy.sum(
        #     (self.P_glrlm / ((ivector[None, :, None, None]**2) *
        #                      (jvector[None, None, :, None]**2))), (1, 2)) / Nr
        # return numpy.nanmean(srlgle, 1)
        res = []
        for _i in range(len(self.P_glrlm)):
          srlgle = numpy.sum(
              (self.P_glrlm[_i] / ((ivector[_i][None, :, None, None]**2) *
                                   (jvector[_i][None, None, :, None]**2))), (1, 2)) / Nr[_i]
          res.append(numpy.nanmean(srlgle, 1))
        return numpy.array(res).astype('float')
    ...
    **14. Short Run High Gray Level Emphasis (SRHGLE)**
    ...
    """
        ...
        # srhgle = numpy.sum((self.P_glrlm * (ivector[None, :, None, None]**2) /
        #                     (jvector[None, None, :, None]**2)), (1, 2)) / Nr
        # return numpy.nanmean(srhgle, 1)
        res = []
        for _i in range(len(self.P_glrlm)):
          srhgle = numpy.sum((self.P_glrlm[_i] * (ivector[_i][None, :, None, None]**2) /
                              (jvector[_i][None, None, :, None]**2)), (1, 2)) / Nr[_i]
          res.append(numpy.nanmean(srhgle, 1))
        return numpy.array(res).astype('float')
    ...
    **15. Long Run Low Gray Level Emphasis (LRLGLE)**
    ...
    """
        ...
        # lrlgle = numpy.sum((self.P_glrlm * (jvector[None, None, :, None]**2) /
        #                     (ivector[None, :, None, None]**2)), (1, 2)) / Nr
        # return numpy.nanmean(lrlgle, 1)
        res = []
        for _i in range(len(self.P_glrlm)):
          lrlgle = numpy.sum((self.P_glrlm[_i] * (jvector[_i][None, None, :, None]**2) /
                              (ivector[_i][None, :, None, None]**2)), (1, 2)) / Nr[_i]
          res.append(numpy.nanmean(lrlgle, 1))
        return numpy.array(res).astype('float')
    ...
    **16. Long Run High Gray Level Emphasis (LRHGLE)**
    ...
    """
        ...
        # lrhgle = numpy.sum(
        #     (self.P_glrlm * ((jvector[None, None, :, None]**2) *
        #                      (ivector[None, :, None, None]**2))), (1, 2)) / Nr
        # return numpy.nanmean(lrhgle, 1)
        res = []
        for _i in range(len(self.P_glrlm)):
          lrhgle = numpy.sum(
              (self.P_glrlm[_i] * ((jvector[_i][None, None, :, None]**2) *
                                   (ivector[_i][None, :, None, None]**2))), (1, 2)) / Nr[_i]
          res.append(numpy.nanmean(lrhgle, 1))
        return numpy.array(res).astype('float')
