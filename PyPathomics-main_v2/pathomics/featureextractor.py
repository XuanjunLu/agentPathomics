    ...
    def execute(self,
        ...
        if isinstance(maskFilepath, np.ndarray) or isinstance(
            ...
            mask = self.loadAutoMask(image, maskFilepath, generalInfo,
                                     **_settings)
        elif isinstance(maskFilepath, list):
            masks = []
            for mask in maskFilepath:
                if isinstance(mask, np.ndarray) or isinstance(
                        mask, six.string_types) and os.path.isfile(mask):
                    mask = self.load2DMask(image, mask, generalInfo,
                                           **_settings)
                elif isinstance(mask, six.string_types):
                    mask = self.loadAutoMask(image, mask, generalInfo,
                                             **_settings)
                masks.append(mask)
            mask = masks
        else:
        ...
        for featureClassName, featureNames in six.iteritems(enabledFeatures):
            ...
            if featureClassName in featureClasses:
                ...
                for (featureName,
                    ...
                    newFeatureName = '%s_%s' % (featureClass.name, featureName)
