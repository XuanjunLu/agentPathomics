...
def performDAG(obofilepath, goTerms, highlightColor, fileName):
    ...
    filePath = os.path.join(os.path.dirname(__file__), 'go-basic.obo')
...
def generate_clinical_phenotype(gene_list_dir, feature_id, clinical_information, clinical_col, save_dir, category=False):
    ### read
    clinical_information = Path(clinical_information)
    ...
    required_columns = ['wsi_id', 'survival_time', 'event_status', 'label', clinical_col]
    for col in required_columns:
        if col not in clinical_information.columns.to_list():
            raise ValueError(f'Please ensure the following columns exist: "wsi_id", "survival_time",'
                             f'"event_status", "label", "{clinical_col}".')
    ...
    unique_vals = sorted(clinical_information[clinical_col].dropna().astype(int).unique())  # [1,2,3,4]
    # interpolation
    ...
    if "/result" in gene_list_dir:
        base_dir = gene_list_dir.split("/result")[0]
    else:
        base_dir = gene_list_dir.split("\\result")[0]
    long_term_feature_matrix = pd.read_csv(f"{base_dir}/long_term/long_term_{feature_id}.csv")
    short_term_feature_matrix = pd.read_csv(f"{base_dir}/short_term/short_term_{feature_id}.csv")
    # group label
    long_term_feature_matrix["group"] = "Low risk"
    short_term_feature_matrix["group"] = "High risk"
    ...
    if category:              # bar plot
        ...
        ## txt report prompt
        report_lines = []
        report_lines.append(f"Analysis of differences in {clinical_col} between high and low-risk groups")
        ...
        report_lines.append(f"Patients in the low-risk group tend to be in which category of {clinical_col}? "
                            f"Conversely, patients in the high-risk group tend to be in which category "
                            f"of {clinical_col}?")
        report_lines.append(f"Tips: In GSEA, if ES > 0, the pathway is upregulated in high-risk patients "
                            f"and downregulated in low-risk patients.")
        ...
        stat, p_value = mannwhitneyu(
            group_feature_matrix[group_feature_matrix["group"] == "Low risk"][clinical_col],
            group_feature_matrix[group_feature_matrix["group"] == "High risk"][clinical_col],
        ...
        report_lines.append(f"For patients in the low-risk group, is the {clinical_col} higher or lower?"
                            f" Is its variability larger or smaller? Conversely, for patients in the high-risk group, "
                            f"is the {clinical_col} higher or lower? Is its variability larger or smaller?")
        report_lines.append(f"Tips: In GSEA, if ES > 0, the pathway is upregulated in high-risk patients "
                            f"and downregulated in low-risk patients.")
        with open(f"{save_dir}/{feature_id}/{clinical_col}_statistics.txt", 'w', encoding='utf-8') as f:
            f.write('\n'.join(report_lines))
...
def enrichment_analysis(deg_res, save_dir, sources, clinical_information, clinical_col, category=False):
    """
    ...
    for gene_list in gene_list_path:
        ...
        generate_clinical_phenotype(gene_list, feature_id, clinical_information, clinical_col, save_dir,
                                    category)
        ...
        eGODataFrame.to_csv(os.path.join(save_dir_feature, f'GO_enrichment_results_PyPathomics_fpkm.csv'), index=False)
        # Generate DAG visualization
        goTerms = eGODataFrame["native"].head(1)
        highlightColor = "#C0392BFF"  # Highlight color
        performDAG(obofilepath, goTerms, highlightColor, os.path.join(save_dir_feature, "DAG_multiple_terms.png"))
        ...
        preRES = performGSEA(sortedGeneDf, outdir=os.path.join(save_dir_feature, 'GSEA_PyPathomics_fpkm'))
...
# Additional function definitions used in main()
...
if __name__ == "__main__":
    ...
    # deg_res = "/home/linjianwei/PycharmProjects/Project_2024_0416_PyPathomics/PyPathomics-main/pathomics_prepare/1319263-8/yihuishan/degs"
    # save_dir = "/home/linjianwei/PycharmProjects/Project_2024_0416_PyPathomics/PyPathomics-main/pathomics_prepare/1319263-8/yihuishan/FEA_GSEA"
    deg_res = r'D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\1757317633337/degs'
    save_dir = r'D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\1757317633337/FEA_GSEA'

    sources = 'GO:All'
    # clinical_information = "/home/linjianwei/PycharmProjects/Project_2024_0416_PyPathomics/PyPathomics-main/pathomics_prepare/1319263-8/extract_wsi_V4/clinical_information.csv"
    clinical_information = r'D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\clinical_information.csv'
    clinical_col = "T_stage"          # "T_stage"   "TNM_stage"   "ES_grade"
    enrichment_analysis(deg_res, save_dir, sources, clinical_information, clinical_col, False)
