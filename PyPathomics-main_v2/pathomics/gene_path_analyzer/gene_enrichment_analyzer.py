...
def performGSEA(sortedGeneDf,
                ...
                seed=42):
    """
    Perform GSEA analysis using the prerank method from gseapy.
...
def main():
    save_dir = "/home/yuxin/bme/pypathomics/gene_test/gene_enrich"
    os.makedirs(save_dir, exist_ok=True)
    # Usage of performFEA function
    geneListFilePath = "/home/yuxin/bme/pypathomics/gene_output/signif_gene(fpkm).xlsx"
    obofilepath = "/home/yuxin/bme/pypathomics/gene_output/go-basic.obo"
    geneDf = pd.read_excel(geneListFilePath)
    geneList = geneDf["new id"].tolist()
    eGODataFrame = performFEA(geneList)
    print(eGODataFrame)
    # Save results to a file
    eGODataFrame.to_csv(os.path.join(save_dir, 'GO_enrichment_results(PyPathomics_fpkm).csv'), index=False)

    # Generate DAG visualization
    goTerms = ['GO:0005178']  # Replace with desired GO terms
    highlightColor = "#C0392BFF"  # Highlight color
    performDAG(obofilepath, goTerms, highlightColor, os.path.join(save_dir, "DAG_multiple_terms.png"))
    ...
    performCreateBarplot(
        ...
        title="GO Functional Enrichment",
        facetGrid='source',
    ...
    enrichmentDf = enrichmentDf.iloc[0:5]
    geneListDf = pd.read_excel(geneListFilePath)
    wideFormatDf = createWideFormatDf(enrichmentDf, geneListDf, fillNA=0)
    print(wideFormatDf.head())
    generateRidgePlot(wideFormatDf, height=500, width=800,
        save_dir=save_dir)

    # Perform GSEA analysis
    geneDf = pd.read_excel(geneListFilePath)
    geneDf = geneDf[['new id', 'logFC']]
    sortedGeneDf = geneDf.sort_values(by='logFC', ascending=False)
    preRES = performGSEA(sortedGeneDf, outdir=os.path.join(save_dir, 'GSEA(PyPathomics_fpkm)'))
    ...
    gseaPlot = GSEAPlotly(
        ...
        title=term,
        fileName=os.path.join(save_dir, "GSEAPlotly.png")
    )
    gseaPlot.show()
    gseaPlot.savefig()
    ...
    enrichmentDf = enrichmentDf.iloc[0:5]
    geneListDf = pd.read_excel(geneListFilePath)
    geneListDf = geneListDf[['new id', 'logFC']]
