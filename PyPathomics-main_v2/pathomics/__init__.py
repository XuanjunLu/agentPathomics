...
def getFeatureClasses():
  ...
  """
    ...
    if _featureClasses is None:  # On first call, enumerate possible feature classes and import PyPathomics modules
        ...
        types = ['shape', 'texture', 'topological', 'interplay']
        for type in types:
            for _, mod, _ in pkgutil.iter_modules([base_dir/type]):
                if str(mod).startswith(
                        '_'
                ):  # Skip loading of 'private' classes, these don't contain feature classes
                    continue
                __import__(f'pathomics.{type}.{mod}')
                module = sys.modules[f'pathomics.{type}.{mod}']
...
cShape = None
try:
    # 使用相对导入，避免在某些运行方式下触发“部分初始化模块”的循环导入问题
    from . import _cmatrices as cMatrices  # noqa: F401
    from . import _cshape as cShape  # noqa: F401
except ImportError as e:
    # 在 Windows/未编译情况下，允许降级到纯 Python 实现（速度会慢一些）
    # 如需强制要求 C 扩展，可设置环境变量 PATHOMICS_REQUIRE_CEXT=1
    require_cext = os.getenv("PATHOMICS_REQUIRE_CEXT", "").strip() in ("1", "true", "True", "YES", "yes")
    if require_cext:
        logger.critical(
            'Unable to load C extensions and PATHOMICS_REQUIRE_CEXT=1. '
            'Please run "python setup.py build_ext --inplace" and retry.',
            exc_info=True,
        )
        raise e

    # 避免在多进程/多次 import 时反复刷屏：只在主进程提示一次（子进程继承环境变量后将不再重复提示）
    suppress_cext_warn = os.getenv("PATHOMICS_SUPPRESS_CEXT_WARNING", "").strip() in ("1", "true", "True", "YES", "yes")
    already_warned = os.getenv("PATHOMICS_CEXT_WARNING_EMITTED", "").strip() == "1"
    is_child_process = False
    try:
        import multiprocessing as _mp  # noqa: WPS433

        is_child_process = _mp.current_process().name != "MainProcess"
    except Exception:
        is_child_process = False

    if (not suppress_cext_warn) and (not already_warned) and (not is_child_process):
        logger.warning(
            'Unable to load C extensions; falling back to pure Python. '
            'If you want better performance, run: "python setup.py build_ext --inplace".'
        )
        # 通过环境变量让后续 spawn 出来的子进程也不会重复提示
        os.environ["PATHOMICS_CEXT_WARNING_EMITTED"] = "1"
    cMatrices = None
    cShape = None
