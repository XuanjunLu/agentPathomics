...
from pathomics.model_constructor.Bclassifiers import QDA_, LDA_, RandomForestC_, SGDClassifier_, DecisionTree_, \
                        KNeighbors_, LinearSVC_, SVC_rbf_, GaussianProcess_, \
                            MLPClassifier_, AdaBoostClassifier_, GaussianNB_, Ridge_
from pathomics.model_constructor.Bfeatures_selection import (UnivariateFeatureSelection, mutual_info_selection, mrmr_selection, \
...
from pathomics.model_constructor.survival_analysis_plotterV2 import SurvivalAnalysisPlotter
...
def ROC_Chart_set(n_feat_sel, nclass, all_roc_fpr, all_roc_tpr,
    ...
    # 当 n_feat_sel==1 或 nclass==1 时，matplotlib 会返回 1D axes；统一转成 2D，避免 axes[i, j] 报错
    axes = np.asarray(axes)
    if n_feat_sel == 1 and nclass == 1:
        axes = axes.reshape(1, 1)
    elif n_feat_sel == 1:
        axes = axes.reshape(1, nclass)
    elif nclass == 1:
        axes = axes.reshape(n_feat_sel, 1)
    b = 0
...
def violin_plot(n_row, n_col, topk_features, features_matrix,
    ...
    # n_row==1 或 n_col==1 时保证 axes 可用 axes[row, col] 访问
    axes = np.asarray(axes)
...
def radar_chart_set(nsel, nclass, feature_selectedrate_dict, save_path,
    ...
    for row in range(nsel):
        for col in range(nclass):
            ...
            axs2 = fig2.add_subplot(111, projection='polar')
            features = list(feature_selectedrate_dict[e].keys())
            if len(features) < 3:
                warnings.warn("The number of feature family needs to be ≥ 3")
            counts = list(feature_selectedrate_dict[e].values())
            ...
            theta = np.concatenate((theta, [theta[0]]))
            # subplot
            axs2.fill(theta, counts, 'b', alpha=0.25)
            axs2.set_title(f'({list_feats_selection_args[row]}, {list_classifers_args[col]})', fontsize=11)
            axs2.set_xticks(theta[:-1])
            axs2.set_xticklabels(features)
            # for i, count in enumerate(counts[:-1]):
            #     angle = theta[i] + (theta[i + 1] - theta[i]) / 2
            #     radius = count + 0.4
            #
            #     formatted_count = "{:.1%}".format(count)
            #     axs2.text(angle, radius, formatted_count, ha='center', va='top')

            plt.savefig(f'{save_path}/{list_feats_selection_args[row]}_{list_classifers_args[col]}_Radar_charts.png')
            plt.close()
            e += 1
...
def kfold_classifier_featureselection(list_feats_selection, list_classifers, save_dir=None, kfold=10,
                                      ...
                                      dealWithImbalance=True, findBestParams=False, feature_score_method='addone'):
    ...
    for i in range(len(list_feats_selection)):
        ...
        for j in range(len(list_classifers)):
            ...
            # plt.show()
            folder_time1 = datetime.now().strftime("%Y-%m-%d_%I-%M-%S_%p")
            plt.savefig(f'{save_dir}/{feats_selection_name}_{classifiers_name}_{folder_time1}_ROC_Curve.png')
            plt.close()
...
def cross_validation(dataset_feature_matrix, survival_info_dir=None, save_dir=None, top_feature_num=6, k_fold=5,
                     feature_score_method="addone", var_thresh=0, corr_threshold=0.9, repeats_num=100,
                     list_feats_selection_args=None,
                     list_classifers_args=None,
                     n_workers=10):
    """
    ...
    list_feats_selection = [list_feats_selection_Dict[list_feats_selection_args[i]] for i in
                            range(len(list_feats_selection_args))]
    ...
    if len(df_features_labels) < 16:
        warnings.warn("Please ensure an adequate number of samples (＞16) and balanced categories as much as possible "
                      "to guarantee the smooth implementation of multi-fold cross-validation")
    ...
    print('0: ', zeros_num.shape[0], '1: ',
          df_labels.shape[0] - zeros_num.shape[0])  #### .shape[0] return the num of row
    ...
    for col in required_columns:   
        if col not in survival_info.columns:
    ...
    all_the_best_y_test_pred_full_with_time = pd.merge(all_the_best_y_test_pred_full, survival_info,
                                                       on='wsi_id', how='inner')
    all_the_best_y_test_pred_full_with_time.to_csv(f'{save_dir}/all_the_best_y_test_pred_full_with_time.csv',
                                                   index=False)
    all_the_best_y_score_pred_full_with_time = pd.merge(all_the_best_y_score_pred_full, survival_info,
                                                        on='wsi_id', how='inner')
    all_the_best_y_score_pred_full_with_time.to_csv(f'{save_dir}/all_the_best_y_score_pred_full_with_time.csv',
                                                    index=False)
    ...
    plotter = SurvivalAnalysisPlotter(
        cross_validation=save_dir,
        list_feats_selection=list_feats_selection_args,
        list_classifiers=list_classifers_args,
        n_workers=3
    )

    # Plot KM curve for every combination
    plotter.km_curve_every(save_dir)
...
if __name__ == '__main__':
    # dataset_feature_matrix = Path('../../example_folder/aggregation/dataset_feature_matrix.csv')
    # survival_info_dir = Path("../../example_folder/survival_info.csv")
    # save_dir = Path('../../example_folder/cross_validation')
    dataset_feature_matrix = Path(r'D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\fc3e0f1b-7f02-11f0-a955-047c16784efc\aggregation\dataset_feature_matrix.csv')
    survival_info_dir = Path(r"D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\survival_info.csv")
    save_dir = Path(r'D:\work_space\AI\PyPathomics-mainv4\PyPathomics-main\example_folder\fc3e0f1b-7f02-11f0-a955-047c16784efc\cross_validation')
    best_combination = cross_validation(dataset_feature_matrix, survival_info_dir, save_dir, top_feature_num=6,
                                        k_fold=5, feature_score_method="addone", var_thresh=0, corr_threshold=0.9,
                                        repeats_num=100,
