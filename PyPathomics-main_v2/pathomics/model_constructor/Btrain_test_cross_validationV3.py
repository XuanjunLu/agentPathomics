...
from pathomics.model_constructor.Bclassifiers import (
    QDA_,
    LDA_,
    RandomForestC_,
    SGDClassifier_,
...
from pathomics.model_constructor.Bfeatures_selection import (
    UnivariateFeatureSelection,
    mutual_info_selection,
    mrmr_selection,
    ttest_selection,
    ranksums_selection,
    varianceThreshold_rm,
    rfe_selection,
    chi_select,
    elastic_net_slectiion,
    random_forest_slectiion,
    XGBoost_slectiion,
...
def ROC_Chart_set(n_feat_sel, nclass, all_roc_fpr, all_roc_tpr,
    ...
    """
    V3：只保存“最佳组合”的 ROC_curve.png（不再保存所有组合拼图）。
    """

    def _first_int(v):
        try:
            # pandas Index / numpy array / list 取第一个
    ...
    i_best = _first_int(max_row_ind)
    j_best = _first_int(max_col_ind)
    b_best = i_best * int(nclass) + j_best

    roc_fpr = all_roc_fpr.iloc[:, b_best]
    roc_tpr = all_roc_tpr.iloc[:, b_best]
    roc_tpr_fill_std = all_roc_tpr_std.iloc[:, b_best]
    auc_value = float(str(df_auc_table.iloc[i_best, j_best]).split('/')[0])
    ...
    ax.plot(roc_fpr_sorted, roc_tpr_sorted, label='ROC', color='darkorange', lw=2)
    ax.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
    ax.fill_between(
        roc_fpr_sorted,
        roc_tpr_sorted - roc_tpr_fill_std_sorted,
        roc_tpr_sorted + roc_tpr_fill_std_sorted,
        color='skyblue',
        alpha=0.5,
        label='Std',
    )
    ax.text(0.95, 0.05, "AUC={:.3f}".format(auc_value), transform=ax.transAxes, fontsize=10,
            va='bottom', ha='right')
    ax.set_title(f'({list_feats_selection_args[i_best]}, {list_classifers_args[j_best]})', fontsize=11)
...
def radar_chart_set(nsel, nclass, feature_selectedrate_dict, save_path,
                    list_feats_selection_args, list_classifers_args, max_row_ind, max_col_ind):
    """
    V3：只保存“最佳组合”的 Radar_charts.png（不再保存所有组合拼图）。
    这里复用 best 组合定位：feature_selectedrate_dict 的顺序与 (row,col) 展开一致。
    """
    ...
    i_best = _first_int(max_row_ind)
    j_best = _first_int(max_col_ind)
    e_best = i_best * int(nclass) + j_best

    # feature_selectedrate_dict 在本实现里通常是 dict（key 为展开后的组合序号 add=0..nsel*nclass-1）
    # 也兼容 list/tuple
    best_dict = None
    if isinstance(feature_selectedrate_dict, dict):
        best_dict = feature_selectedrate_dict.get(e_best)
        if best_dict is None and len(feature_selectedrate_dict) > 0:
            # 兜底：取第一个 key 对应的值
            first_key = next(iter(sorted(feature_selectedrate_dict.keys())))
            best_dict = feature_selectedrate_dict[first_key]
    elif isinstance(feature_selectedrate_dict, (list, tuple)):
        if len(feature_selectedrate_dict) == 0:
            raise ValueError("feature_selectedrate_dict 为空，无法绘制雷达图")
        if e_best >= len(feature_selectedrate_dict):
            e_best = 0
        best_dict = feature_selectedrate_dict[e_best]
    else:
        raise TypeError(f"feature_selectedrate_dict 类型不支持: {type(feature_selectedrate_dict)}")

    features = list(best_dict.keys())
    if len(features) < 3:
        warnings.warn("The number of feature family needs to be ≥ 3")
    counts = list(best_dict.values())

    theta = np.linspace(0, 2 * np.pi, len(features), endpoint=False)
    counts = np.concatenate((counts, [counts[0]]))
    theta = np.concatenate((theta, [theta[0]]))

    fig2, ax = plt.subplots(nrows=1, ncols=1, figsize=(6, 5), subplot_kw=dict(polar=True))
    ax.fill(theta, counts, 'b', alpha=0.25)
    ax.set_title(
        f'({list_feats_selection_args[i_best]}, {list_classifers_args[j_best]})',
        fontsize=12,
    )
        ...
        for j in range(len(list_classifers)):
            ...
            if n_workers:
                try:
                    ...
                    # Windows 某些环境（例如映射盘/权限受限/多进程受限）下可能触发
                    # OSError('GetModuleFileNameEx failed')；此处降级为单进程以保证流程可跑通
                    warnings.warn(
                        f"multiprocessing Pool failed ({e}); falling back to single-process execution.",
                        RuntimeWarning,
                    )
                    results_pack = []
                    for _ in range(n_repeats):
                        y_test_pred_full, y_test_score_pred_full, selected_features_list0, features_scores0, best_params = run(
                            X,
                            y,
                            selected_features_coorelated_feats,
                            i,
                            j,
                            list_feats_selection,
                            list_classifers,
                            n_features,
                            kfold,
                            findBestParams,
                            dealWithImbalance,
                            feature_score_method,
                        )
                        results_pack.append(
                            [y_test_pred_full, y_test_score_pred_full, selected_features_list0, features_scores0, best_params]
                        )
            else:
                # compute features normally (single-process)
                results_pack = []
                for _ in range(n_repeats):
                    y_test_pred_full, y_test_score_pred_full, selected_features_list0, features_scores0, best_params = run(
                        X,
...
def cross_validation(dataset_feature_matrix: Path, survival_info_dir: Path, save_dir: Path, top_feature_num=6, k_fold=5,
    ...
    radar_chart_set(len(list_feats_selection_args), len(list_classifers_args), three_features_dict,
                    results_to_save_path, list_feats_selection_args, list_classifers_args, max_row_ind, max_col_ind)
    # ROC，5*11
    ...
    # 生存分析（KM 曲线）：对齐 V2 行为，便于 GUI/下游直接使用
