...
# 包内导入：保证以 `import pathomics...` 方式运行时也能找到模块
from pathomics.model_constructor.Bclassifiers import (  # noqa: E402
    QDA_,
    LDA_,
    RandomForestC_,
    SGDClassifier_,
...
from pathomics.model_constructor.Bfeatures_selection import (  # noqa: E402
...
class SurvivalAnalysisPlotter:
    def __init__(self, cross_validation: Path, list_feats_selection=None, list_classifiers=None, n_workers=4):
        ...
        # 兼容不同版本的产物命名：
        # - 旧版(V2)常输出：all_the_best_y_test_pred_full_with_time.csv
        # - 新版(V3)常输出：all_the_y_test_pred_full_with_time.csv
        cand_best = self.cross_validation / "all_the_best_y_test_pred_full_with_time.csv"
        cand_v3 = self.cross_validation / "all_the_y_test_pred_full_with_time.csv"
        self.data_file = cand_best if cand_best.exists() else cand_v3
