...
# 计算边界点的傅里叶描述算子，将边界点x+y转换为复数x+yj，输出z的长度和边界点xy的数量一致
def frdescp(s_):
...
# 将一个边界点的集合转换为二值图（binary image），其中边界点的位置被标记为 1，其它位置为 0
def bound2im(b):
...
def compute_m(F):
    ...
    m['m10'] = np.sum(x * F)
    m['m01'] = np.sum(y * F)
    m['m11'] = np.sum(x * y * F)        # xy
    m['m20'] = np.sum(x * x * F)        # x2
    m['m02'] = np.sum(y * y * F)        # y2
    m['m30'] = np.sum(x * x * x * F)    # x3
    m['m03'] = np.sum(y * y * y * F)    # y3
    m['m12'] = np.sum(x * y * y * F)    # xy2
    m['m21'] = np.sum(x * x * y * F)    # x2y
    return m
...
def extract_morph_feats(bounds):
    ...
    feats_stats[feats_stats == np.inf] = np.nan       ## change ±inf to np.nan
