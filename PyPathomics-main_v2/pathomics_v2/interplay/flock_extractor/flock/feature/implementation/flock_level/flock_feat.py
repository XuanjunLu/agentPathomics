...
from pathomics.interplay.flock_extractor.flock.core import FlockCore, FlockTyping, ClusterSkeleton
from pathomics.interplay.flock_extractor.flock.feature.implementation.cluster_level.cluster import FlockPolygons
from lazy_property import LazyProperty
from typing import Dict, Union, Sequence, Any

from pathomics.interplay.flock_extractor.flock.feature.util import GraphUtil, JsonDict
from pathomics.interplay.flock_extractor.flock.feature.implementation.flock_level.helper import FeatReduction

from pathomics.interplay.flock_extractor.flock.feature.implementation import FeatureSkeleton, PolygonFeature, FeatureGenerator
from pathomics.interplay.flock_extractor.flock.feature.implementation.flock_level import data_msg
import logging
...
class FlockFeature(FeatureSkeleton):
    ...
    @staticmethod
    def _flatten(in_dict: Dict[str, Any], excluded_keys: Sequence[str], upper_key=None):
        """
        ...
        for k, v in in_dict.items():
            ...
            if not isinstance(v, Dict):
                ...
                sub_keys, sub_vals = FlockFeature._flatten(v, excluded_keys,k)
            out_keys += sub_keys
            out_vals += sub_vals
