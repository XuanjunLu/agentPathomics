...
def main():
    ...
    parser.add_argument('--dataset_feature_matrix', type=Path, default='../../example_folder/aggregation/dataset_feature_matrix.csv', help='Dataset_feature_matrix.csv')
    parser.add_argument('--survival_info_dir', type=Path, default='../../example_folder/survival_info.csv', help='Survival_info.csv')
    parser.add_argument('--save_dir', type=Path, default='../../example_folder/cross_validation')
    args = parser.parse_args()
    best_combination = cross_validation(args.dataset_feature_matrix, args.survival_info_dir, args.save_dir,
                                        top_feature_num=6, k_fold=5, feature_score_method="addone", var_thresh=0,
                                        corr_threshold=0.9, repeats_num=100,
                                        list_feats_selection_args=['Lasso', 'XGBoost', 'RandomForest', 'Elastic-Net', 'RFE',
                                                                   'Univariate', 'mrmr', 'ttest', 'ranksums', 'mutualInfo'],
                                        list_classifers_args=['QDA', 'LDA', 'RandomForest', 'DecisionTree', 'KNeigh',
                                                              'LinearSVC', 'MLP', 'GaussianNB', 'SGD', 'SVC_rbf', 'AdaBoost'],
                                        n_workers=5)
