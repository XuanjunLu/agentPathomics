...
from pathomics import getProgressReporter, imageoperations


class PathomicsFeaturesBase(object):
    """
    ...
    Logging is set up using a child logger from the parent 'pathomics' logger. This retains the toolbox structure in
    the generated log. The child logger is named after the module containing the feature class (e.g. 'pathomics.glcm').
    ...
    .. note::
        ...
        :ref:`pathomics-customization-label`, which includes a comprehensive list of all possible settings, including
    ...
    def __init__(self, inputImage, inputMask, **kwargs):
        ...
        self.rois = measure.label(self.maskArray)
        self.targetROIArray = [self.imageArray[self.rois == i] for i in numpy.unique(self.rois) if i != 0]
        self.roi_masks = [(self.rois == i).astype(numpy.uint8) for i in numpy.unique(self.rois) if i != 0]
        return
    ...
    def _initVoxelBasedCalculation(self):
        ...
        maskArray = sitk.GetArrayFromImage(
    ...
    def bounds2coords(self, bounds):
        x = [bounds[i][0] for i in range(len(bounds))]
        y = [bounds[i][1] for i in range(len(bounds))]
        x_coord = [int(bounds[i][4]) for i in range(len(bounds))]
        y_coord = [int(bounds[i][5]) for i in range(len(bounds))]

        x_y_coord = list(map(list, zip(x_coord, y_coord)))
        return np.array(x_y_coord)
    ...
    def enableFeatureByName(self, featureName, enable=True):
        ...
        if self.featureNames[featureName]:
            self.logger.warning('Feature %s is deprecated, use with caution!',
                                featureName)
    ...
    def enableAllFeatures(self):
        ...
        a call to :py:func:`~pathomics.base.PathomicsBase.enableFeatureByName()`,
    ...
    def _calculateVoxels(self):
        ...
        for feature, enabled in six.iteritems(self.enabledFeatures):
            if enabled:
                self.featureValues[feature] = numpy.full(list(
                    self.inputImage.GetSize())[::-1],
                                                         initValue,
                                                         dtype='float')
        ...
        with self.progressReporter(total=n_batches, desc='batch') as pbar:
            while voxel_batch_idx < voxel_count:
                self.logger.debug('Calculating voxel batch no. %i/%i',
                                  int(voxel_batch_idx / voxelBatch) + 1,
                                  n_batches)
                voxelCoords = self.labelledVoxelCoordinates[:, voxel_batch_idx:
                                                            voxel_batch_idx +
                                                            voxelBatch]
                ...
                for success, featureName, featureValue in self._calculateFeatures(
                        voxelCoords):
                    if success:
                        self.featureValues[featureName][tuple(
                            voxelCoords)] = featureValue
        ...
        for feature, enabled in six.iteritems(self.enabledFeatures):
            if enabled:
                self.featureValues[feature] = sitk.GetImageFromArray(
        ...
        for feature, enabled in six.iteritems(self.enabledFeatures):
            if enabled:
                try:
                    ...
                    yield True, feature, getattr(self, 'get%sFeatureValue' %
                                                 feature)()
                    ...
                    self.logger.debug('Feature %s is deprecated: %s', feature,
