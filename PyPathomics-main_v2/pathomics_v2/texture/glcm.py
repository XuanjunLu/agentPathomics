# this feature is from pyradiomics
import numpy
from six.moves import range

from pathomics import base, cMatrices, cShape, deprecated


class PathomicsGLCM(base.PathomicsFeaturesBase):
    r"""
  ...
  Note that pypathomics by default computes symmetrical GLCM!
  ...
  """
    ...
    def __init__(self, inputImage, inputMask, **kwargs):
        super(PathomicsGLCM, self).__init__(inputImage, inputMask, **kwargs)
        self.name = 'GLCM'
    ...
    def _initCalculation(self, voxelCoordinates=None):
        ...
        self.logger.debug(
            'GLCM feature class initialized, calculated GLCM with shape %s of each ROI',
            self.P_glcm[0].shape)
    ...
    """
        ...
        for _i in range(len(self.roi_masks)):
          ...
          matrix_args = [
              self.imageArray, self.roi_masks[_i],
              numpy.array(self.settings.get('distances', [1])), Ng,
              self.settings.get('force2D', False),
              # self.settings.get('force2D', True),
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          ...
          px = self.P_glcm[_i].sum(2, keepdims=True)
          # marginal column probabilities #shape = (Nv, 1, Ng, angles)
          py = self.P_glcm[_i].sum(1, keepdims=True)

          # shape = (Nv, 1, 1, angles)
          ux = numpy.sum(i[None, :, :, None] * self.P_glcm[_i], (1, 2),
                        keepdims=True)
          uy = numpy.sum(j[None, :, :, None] * self.P_glcm[_i], (1, 2),
                        keepdims=True)

          # shape = (Nv, 2*Ng-1, angles)
          pxAddy = numpy.array([
              numpy.sum(self.P_glcm[_i][:, i + j == k, :], 1) for k in kValuesSum
          ]).transpose((1, 0, 2))
          # shape = (Nv, Ng, angles)
          pxSuby = numpy.array([
              numpy.sum(self.P_glcm[_i][:, numpy.abs(i - j) == k, :], 1)
              for k in kValuesDiff
          ]).transpose((1, 0, 2))

          # shape = (Nv, angles)
          HXY = (-1) * numpy.sum((self.P_glcm[_i] * numpy.log2(self.P_glcm[_i] + eps)),
                                (1, 2))
          
          kValuesSums.append(kValuesSum)
          kValuesDiffs.append(kValuesDiff)
          pxs.append(px)
          pys.append(py)
          uxs.append(ux)
          uys.append(uy)
          pxAddys.append(pxAddy)
          pxSubys.append(pxSuby)
          HXYs.append(HXY)     
        ...
        self.coefficients['kValuesSum'] = kValuesSums
        self.coefficients['kValuesDiff'] = kValuesDiffs
        self.coefficients['px'] = pxs
        self.coefficients['py'] = pys
        self.coefficients['ux'] = uxs
        self.coefficients['uy'] = uys
        self.coefficients['pxAddy'] = pxAddys
        self.coefficients['pxSuby'] = pxSubys
        self.coefficients['HXY'] = HXYs
    ...
    **1. Autocorrelation**
    ...
    """
        ...
        acs = []
        for _i in range(len(self.P_glcm)):
          ac = numpy.sum(self.P_glcm[_i] * (i * j)[None, :, :, None], (1, 2))
          acs.append(ac)
        
        res = [numpy.nanmean(ac, 1) for ac in acs]
    ...
    **2. Joint Average**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glcm)):
            res.append(self.coefficients['ux'][_i].mean((1,2,3)))
    ...
    **3. Cluster Prominence**
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          cp = numpy.sum(
              (self.P_glcm[_i] * (((i + j)[None, :, :, None] - ux[_i] - uy[_i])**4)), (1, 2))
          cps.append(cp)
        # cp = numpy.sum(
        #     (self.P_glcm * (((i + j)[None, :, :, None] - ux - uy)**4)), (1, 2))
        res = [numpy.nanmean(cp, 1) for cp in cps]
    ...
    **4. Cluster Shade**
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          cs = numpy.sum(
              (self.P_glcm[_i] * (((i + j)[None, :, :, None] - ux[_i] - uy[_i])**3)), (1, 2))
          css.append(cs)
        res = [numpy.nanmean(cs, 1) for cs in css]
        return numpy.array(res).astype('float')
    ...
    **5. Cluster Tendency**
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          ct = numpy.sum(
              (self.P_glcm[_i] * (((i + j)[None, :, :, None] - ux[_i] - uy[_i])**2)), (1, 2))
          cts.append(ct)
        res = [numpy.nanmean(ct, 1) for ct in cts]
        return numpy.array(res).astype('float')
    ...
    **6. Contrast**
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          cont = numpy.sum(
              (self.P_glcm[_i] * ((numpy.abs(i - j))[None, :, :, None]**2)), (1, 2))
          conts.append(cont)
        res = [numpy.nanmean(cont, 1) for cont in conts]
        return numpy.array(res).astype('float')
    ...
    **7. Correlation**
    ...
    """
        ...
        # # shape = (Nv, 1, 1, angles)
        # sigx = numpy.sum(self.P_glcm * ((i[None, :, :, None] - ux)**2), (1, 2),
        #                  keepdims=True)**0.5
        # # shape = (Nv, 1, 1, angles)
        # sigy = numpy.sum(self.P_glcm * ((j[None, :, :, None] - uy)**2), (1, 2),
        #                  keepdims=True)**0.5

        # corm = numpy.sum(self.P_glcm * (i[None, :, :, None] - ux) *
        #                  (j[None, :, :, None] - uy), (1, 2),
        #                  keepdims=True)
        # corr = corm / (sigx * sigy + eps)
        # corr[sigx *
        #      sigy == 0] = 1  # Set elements that would be divided by 0 to 1.
        # return numpy.nanmean(corr, (1, 2, 3))
        corrs = []
        for _i in range(len(self.P_glcm)):
          sigx = numpy.sum(self.P_glcm[_i] * ((i[None, :, :, None] - ux[_i])**2), (1, 2),
                          keepdims=True)**0.5
          sigy = numpy.sum(self.P_glcm[_i] * ((j[None, :, :, None] - uy[_i])**2), (1, 2),
                          keepdims=True)**0.5

          corm = numpy.sum(self.P_glcm[_i] * (i[None, :, :, None] - ux[_i]) *
                          (j[None, :, :, None] - uy[_i]), (1, 2),
                          keepdims=True)
          corr = corm / (sigx * sigy + eps)
          corr[sigx *
              sigy == 0] = 1
          corrs.append(corr)
        res = [numpy.nanmean(corr, (1, 2, 3)) for corr in corrs]
        return numpy.array(res).astype('float')
    ...
    **8. Difference Average**
    ...
    """
        ...
        diffavgs = []
        for _i in range(len(self.P_glcm)):
          diffavg = numpy.sum((kValuesDiff[_i][None, :, None] * pxSuby[_i]), 1)
          diffavgs.append(diffavg)
        res = [numpy.nanmean(diffavg, 1) for diffavg in diffavgs]
        return numpy.array(res).astype('float')
    ...
    **9. Difference Entropy**
    ...
    """
        ...
        difents = []
        for _i in range(len(self.P_glcm)):
          difent = (-1) * numpy.sum((pxSuby[_i] * numpy.log2(pxSuby[_i] + eps)), 1)
          difents.append(difent)
        res = [numpy.nanmean(difent, 1) for difent in difents]
        return numpy.array(res).astype('float')
    ...
    **10. Difference Variance**
    ...
    """
        pxSuby = self.coefficients['pxSuby']
        kValuesDiff = self.coefficients['kValuesDiff']
        # diffavg = numpy.sum((kValuesDiff[None, :, None] * pxSuby),
        #                     1,
        #                     keepdims=True)
        # diffvar = numpy.sum(
        #     (pxSuby * ((kValuesDiff[None, :, None] - diffavg)**2)), 1)
        # return numpy.nanmean(diffvar, 1)
        diffvars = []
        for _i in range(len(self.P_glcm)):
          diffavg = numpy.sum((kValuesDiff[_i][None, :, None] * pxSuby[_i]),
                              1,
                              keepdims=True)
          diffvar = numpy.sum(
              (pxSuby[_i] * ((kValuesDiff[_i][None, :, None] - diffavg)**2)), 1)
          diffvars.append(diffvar)
        res = [numpy.nanmean(diffvar, 1) for diffvar in diffvars]
        return numpy.array(res).astype('float')
    ...
    **DEPRECATED. Dissimilarity**
    ...
    .. warning::
      ...
      :py:func:`~pathomics.glcm.PathomicsGLCM.getDifferenceAverageFeatureValue()`.
      See :ref:`here <pathomics-excluded-dissimilarity-label>` for the proof. **Enabling this feature will result in the
    ...
    """
        raise DeprecationWarning(
            ...
            'see http://pypathomics.readthedocs.io/en/latest/removedfeatures.html for more details'
        )
    ...
    **11. Joint Energy**
    ...
    """
        ...
        enes = []
        for _i in range(len(self.P_glcm)):
          ene = numpy.sum((self.P_glcm[_i]**2), (1, 2))
          enes.append(ene)
        res = [numpy.nanmean(ene, 1) for ene in enes]
        return numpy.array(res).astype('float')
    ...
    **12. Joint Entropy**
    ...
    """
        ...
        res = []
        for _i in range(len(self.P_glcm)):
          ent = self.coefficients['HXY'][_i]
          res.append(numpy.nanmean(ent, 1))
        return numpy.array(res).astype('float')
    ...
    **DEPRECATED. Homogeneity 1**
    ...
    .. warning::
      ...
      :py:func:`~pathomics.glcm.PathomicsGLCM.getIdFeatureValue()`. **Enabling this feature will result in the
      logging of a DeprecationWarning (does not interrupt extraction of other features), no value is calculated for this features**
    """
        raise DeprecationWarning(
    ...
    **DEPRECATED. Homogeneity 2**
    ...
    .. warning::
      ...
      :py:func:`~pathomics.glcm.PathomicsGLCM.getIdmFeatureValue()`. **Enabling this feature will result in the
      logging of a DeprecationWarning (does not interrupt extraction of other features), no value is calculated for this features**
    """
        raise DeprecationWarning(
    ...
    **13. Informational Measure of Correlation (IMC) 1**
    ...
    """
        ...
        # # shape = (Nv, angles)
        # HXY1 = (-1) * numpy.sum((self.P_glcm * numpy.log2(px * py + eps)),
        #                         (1, 2))
        ...
        imc1s = []
        for _i in range(len(self.P_glcm)):
          HX = (-1) * numpy.sum((px[_i] * numpy.log2(px[_i] + eps)), (1, 2))
          HY = (-1) * numpy.sum((py[_i] * numpy.log2(py[_i] + eps)), (1, 2))
          HXY1 = (-1) * numpy.sum((self.P_glcm[_i] * numpy.log2(px[_i] * py[_i] + eps)),
                                  (1, 2))
          ...
          imc1[div == 0] = 0
          imc1s.append(imc1)
        res = [numpy.nanmean(imc1 ,1) for imc1 in imc1s]
        return numpy.array(res).astype('float')
    ...
    **14. Informational Measure of Correlation (IMC) 2**
    ...
    """
        ...
        # shape = (Nv, angles)
        # HXY2 = (-1) * numpy.sum(((px * py) * numpy.log2(px * py + eps)),
        #                         (1, 2))
        ...
        imc2s = []
        for _i in range(len(self.P_glcm)):
          HXY2 = (-1) * numpy.sum(((px[_i] * py[_i]) * numpy.log2(px[_i] * py[_i] + eps)),
                                  (1, 2))

          imc2 = (1 - numpy.e**(-2 * (HXY2 - HXY[_i])))**0.5
          imc2[HXY2 == HXY[_i]] = 0
          imc2s.append(imc2)
        res = [numpy.nanmean(imc2, 1) for imc2 in imc2s]
        return numpy.array(res).astype('float')
    ...
    **15. Inverse Difference Moment (IDM)**
    ...
    """
        ...
        idms = []
        for _i in range(len(self.P_glcm)):
          idm = numpy.sum(pxSuby[_i] / (1 + (kValuesDiff[_i][None, :, None]**2)), 1)
          idms.append(idm)
        res = [numpy.nanmean(idm, 1) for idm in idms]
        return numpy.array(res).astype('float')
    ...
    **16. Maximal Correlation Coefficient (MCC)**
    ...
    """
        ...
        # Q = ((self.P_glcm[:, :, None, 0, :] * self.P_glcm[:, None, :, 0, :])
        #      /  # slice: v, i, j, k, d
        #      (px[:, :, None, 0, :] * py[:, None, :, 0, :] + eps)
        #      )  # sum over k (4th axis --> index 3)

        # for gl in range(1, self.P_glcm.shape[1]):
        #     Q += ((self.P_glcm[:, :, None, gl, :] *
        #            self.P_glcm[:, None, :, gl, :]) /  # slice: v, i, j, k, d
        ...
        # Q_eigenValue.sort(
        # )  # sorts along last axis --> eigenvalues, low to high
        ...
        mccs = []
        for _i in range(len(self.P_glcm)):
          Q = ((self.P_glcm[_i][:, :, None, 0, :] * self.P_glcm[_i][:, None, :, 0, :])
              /  # slice: v, i, j, k, d
              (px[_i][:, :, None, 0, :] * py[_i][:, None, :, 0, :] + eps)
              )
          for gl in range(1, self.P_glcm[_i].shape[1]):
              Q += ((self.P_glcm[_i][:, :, None, gl, :] *
                    self.P_glcm[_i][:, None, :, gl, :]) /  # slice: v, i, j, k, d
                  (px[_i][:, :, None, 0, :] * py[_i][:, None, :, gl, :] + eps)
                  )
          Q_eigenValue = numpy.linalg.eigvals(Q.transpose((0, 3, 1, 2)))
          Q_eigenValue.sort(
          )
          ...
          mccs.append(mcc)
        res = [numpy.nanmean(mcc, 1).real for mcc in mccs]
        return numpy.array(res).astype('float')
    ...
    **17. Inverse Difference Moment Normalized (IDMN)**
    ...
    """
        ...
        for _i in range(len(self.P_glcm)):
          idmn = numpy.sum(
              pxSuby[_i] / (1 + ((kValuesDiff[_i][None, :, None]**2) / (Ng**2))), 1)
          idmns.append(idmn)
        res = [numpy.nanmean(idmn, 1) for idmn in idmns]
        return numpy.array(res).astype('float')
    ...
    **18. Inverse Difference (ID)**
    ...
    """
        ...
        invDiffs = []
        for _i in range(len(self.P_glcm)):
          invDiff = numpy.sum(pxSuby[_i] / (1 + kValuesDiff[_i][None, :, None]), 1)
          invDiffs.append(invDiff)
        res = [numpy.nanmean(invDiff, 1) for invDiff in invDiffs]
        return numpy.array(res).astype('float')
    ...
    **19. Inverse Difference Normalized (IDN)**
    ...
    """
        ...
        idns = []
        for _i in range(len(self.P_glcm)):
          idn = numpy.sum(pxSuby[_i] / (1 + (kValuesDiff[_i][None, :, None] / Ng)), 1)
          idns.append(idn)
        res = [numpy.nanmean(idn, 1) for idn in idns]
        return numpy.array(res).astype('float')
    ...
    **20. Inverse Variance**
    ...
    """
        pxSuby = self.coefficients['pxSuby']
        kValuesDiff = self.coefficients['kValuesDiff']
        # inv = numpy.sum(pxSuby[:, 1:, :] / kValuesDiff[None, 1:, None]**2,
        #                 1)  # Skip k = 0 (division by 0)
        # return numpy.nanmean(inv, 1)
        invs = []
        for _i in range(len(self.P_glcm)):
          inv = numpy.sum(pxSuby[_i][:, 1:, :] / kValuesDiff[_i][None, 1:, None]**2,
                          1)
          invs.append(inv)
        res = [numpy.nanmean(inv, 1) for inv in invs]
        return numpy.array(res).astype('float')
    ...
    **21. Maximum Probability**
    ...
    """
        ...
        maxprobs = []
        for _i in range(len(self.P_glcm)):
          maxprob = numpy.amax(self.P_glcm[_i], (1, 2))
          maxprobs.append(maxprob)
        res = [numpy.nanmean(maxprob, 1) for maxprob in maxprobs]
        return numpy.array(res).astype('float')
    ...
    **22. Sum Average**
    ...
    .. warning::
      ...
      :ref:`here <pathomics-excluded-sumvariance-label>` for the proof that :math:`\text{Sum Average} = \mu_x + \mu_y`.
    ...
    """
        ...
        sumavgs = []
        for _i in range(len(self.P_glcm)):
          sumavg = numpy.sum((kValuesSum[_i][None, :, None] * pxAddy[_i]), 1)
          sumavgs.append(sumavg)
        res = [numpy.nanmean(sumavg, 1) for sumavg in sumavgs]
        return numpy.array(res).astype('float')
    ...
    **DEPRECATED. Sum Variance**
    ...
    .. warning::
      ...
      :py:func:`~pathomics.glcm.PathomicsGLCM.getClusterTendencyFeatureValue()`.
      See :ref:`here <pathomics-excluded-sumvariance-label>` for the proof. **Enabling this feature will result in the
      logging of a DeprecationWarning (does not interrupt extraction of other features), no value is calculated for this features**
    """
        raise DeprecationWarning(
    ...
    **23. Sum Entropy**
    ...
    """
        ...
        sumentrs = []
        for _i in range(len(self.P_glcm)):
          sumentr = (-1) * numpy.sum((pxAddy[_i] * numpy.log2(pxAddy[_i] + eps)), 1)
          sumentrs.append(sumentr)
        res = [numpy.nanmean(sumentr, 1) for sumentr in sumentrs]
        return numpy.array(res).astype('float')
    ...
    **24. Sum of Squares**
    ...
    """
        ...
        sss = []
        for _i in range(len(self.P_glcm)):
          ss = numpy.sum((self.P_glcm[_i] * ((i[None, :, :, None] - ux[_i])**2)), (1, 2))
          sss.append(ss)
        res = [numpy.nanmean(ss, 1) for ss in sss]
        return numpy.array(res).astype('float')
