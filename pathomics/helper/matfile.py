...
def extract_tumorNuclear_coords_from_one_mat(mat_file, target_type):
    mat = sio.loadmat(mat_file)
    coords = dict()

    type_label = mat['inst_type']
    inst_centroid = mat['inst_centroid']
    if isinstance(type_label, np.ndarray):
        if target_type is None:
            coords['inst_centroid'] = inst_centroid  ## get all of the cell coords
        else:
            target_type_idx = np.where(type_label.flatten() == target_type)[0]
            coords[f"type_{target_type}"] = inst_centroid[target_type_idx]
            coords['others'] = np.delete(inst_centroid, target_type_idx, axis=0)
