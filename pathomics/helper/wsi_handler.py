...
class OpenSlideHandler(FileHandler):
    ...
    def __load_metadata(self):
        self.box = None
        if self.path_presplit:
            tree = et.parse(self.path_presplit)
            root = tree.getroot()
            box = np.array([[float(vertex.get('X')), float(vertex.get('Y'))] for vertex in root.find('Annotation').find('Regions').find('Region').find('Vertices').findall('Vertex')])
            self.box = box = box.astype('int64')
            start_point = np.array([box[:, 0].min(), box[:, 1].min()])
            end_point = np.array([box[:, 0].max(), box[:, 1].max()])
            box_size = end_point - start_point
        metadata = {}
        ...
        metadata = [
            ...
            ("mpp  ", mpp),
            ("base_shape", np.array(self.file_ptr.dimensions) if not self.path_presplit else np.array(box_size)),
    ...
    def read_region(self, coords, size):
        ...
        if self.image_ptr is None:
            ...
            if self.box:
                new_coord[0] = int(new_coord[0] + self.box[:, 0].min())
                new_coord[1] = int(new_coord[1] + self.box[:, 1].min())
    ...
    def get_full_img(self, read_mag=None, read_mpp=None):
        ...
        if self.path_presplit:
            ...
            end_point = np.array([box[:, 0].max(), box[:, 1].max()])
            box_size = end_point - start_point
            read_size = (box_size / self.metadata['downsample_level'][read_lv]).astype('int64')
            if scale_factor:
                read_size = (read_size * scale_factor).astype('int64')
        temp = self.metadata['downsample_level'][read_lv]
        wsi_img = self.file_ptr.read_region(start_point, read_lv, read_size)
...
def get_file_handler(path, path_presplit = None):
    backend = path.suffix
    if backend in [
            '.svs', '.tif', 
            '.vms', '.vmu', '.ndpi',
        ...
        return OpenSlideHandler(path, path_presplit)
    else:
