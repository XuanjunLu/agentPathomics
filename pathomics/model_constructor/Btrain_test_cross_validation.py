...
## the methods of feature selection and classifer have been integrated to classes and now ensemble all the class into dictionary
    ...
    for i in range(n_feat_sel):
        for j in range(nclass):
            ...
            axes[i, j].text(0.95, 0.05, "AUC={:.3f}".format(auc_value), transform=axes[i, j].transAxes, fontsize=8,
                            va='bottom', ha='right')
            axes[i, j].set_title(f'({args.list_feats_selection[i]}, {args.list_classifers[j]})', fontsize=11)
...
def violin_plot(n_row, n_col, topk_features, features_matrix,
                ...
                save_path="./visual_select_times") -> None:
    """
...
def imBalanced_MES(estimator, X, y, X_test, y_test, params_dict=None): 
    ...
    y_test_pred_vote, y_test_score_pred_vote = voting(y_test_pred_list, y_test_score_pred_list, vote_type='hard')
...
def radar_chart_set(nsel, nclass, feature_selectedrate_dict, save_path):
    ...
    for row in range(nsel):
        for col in range(nclass):
            ...
            axs2[row, col].set_title(f'({args.list_feats_selection[row]}, {args.list_classifers[col]})', fontsize=11)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(save_path + '/Radar Charts.png')
    plt.close()
...
def run(X, y, selected_features_coorelated_feats, i_featsSelection, j_classifer,
...
def kfold_classifier_featureselection(list_feats_selection, list_classifers, kfold=10,
                                    df_features_training_uncorr=None, df_total_labels=None, df_patients_id=None,
                                    n_repeats=100, n_features=8, n_workers=12,
                                    dealWithImbalance=False, findBestParams=False, feature_score_method='addone'):
    ...
    # all_the_best_y_test_pred_full = pd.concat([df_patients_id, df_total_labels], axis=1)
    # all_the_best_y_score_pred_full = pd.concat([df_patients_id, df_total_labels], axis=1)
    all_the_best_y_test_pred_full = df_patients_id
    all_the_best_y_score_pred_full = df_patients_id
    ...
    for i in range(len(list_feats_selection)):
        ...
        for j in range(len(list_classifers)):
            ...
            fpr_list = []
            tpr_list = []
            features_scores = np.zeros(len(selected_features_coorelated_feats))   # len(selected_features_coorelated_feats)=203
            # kf = KFold(n_splits=kfold, shuffle=True )
            # kf = RepeatedStratifiedKFold(n_splits=kfold, n_repeats=100)
            begin_time = time.time()
            # run 100 times five-fold cross validation
            if n_workers:
            ...
            the_largest_auc = 0
            the_best_y_test_pred_full = None
            the_best_y_test_score_pre_full = None
            for i_pack in range(len(results_pack)):
                ...
                if auc > the_largest_auc:
                    the_largest_auc = auc
                    the_best_y_test_pred_full = y_test_pred_full
                    y_test_score_pred_full = pd.DataFrame(y_test_score_pred_full)
                    y_test_score_pred_full = y_test_score_pred_full.drop(columns=[0])
                    the_best_y_test_score_pre_full = y_test_score_pred_full

            feats_selection_name = str(list_feats_selection[i]).split(' ')[1]
            classifiers_name = str(list_classifers[j]).split('.')[-1][:-2]

            # return the prediction results corresponding to the optimal AUC under all combinations,
            # all of which are stored in all_the_best_y_test_pred_full
            the_best_y_test_pred_full = pd.DataFrame(the_best_y_test_pred_full)
            the_best_y_test_pred_full = the_best_y_test_pred_full.rename(columns=lambda x: f'{feats_selection_name}_{classifiers_name}')
            all_the_best_y_test_pred_full = pd.concat([all_the_best_y_test_pred_full, the_best_y_test_pred_full], axis=1)

            the_best_y_test_score_pre_full = pd.DataFrame(the_best_y_test_score_pre_full)
            the_best_y_test_score_pre_full = the_best_y_test_score_pre_full.rename(columns=lambda x: f'{feats_selection_name}_{classifiers_name}')
            all_the_best_y_score_pred_full = pd.concat([all_the_best_y_score_pred_full, the_best_y_test_score_pre_full], axis=1)
            ...
            folder_time1 = datetime.now().strftime("%Y-%m-%d_%I-%M-%S_%p")
            plt.savefig(
                f'/home/linjianwei/PycharmProjects/Project_LXJ/visual_select_times/{feats_selection_name}_{classifiers_name}_{folder_time1}_ROC_Curve.png')
            plt.close()
            ...
            featfamilyname_list = ['firstorder', 'glcm', 'glrlm']
            ...
            violin_plot(2, 3, top12_features_half, df_features_training_uncorr,
                        df_total_labels, feats_selection_name, classifiers_name,
                        save_path="./visual_select_times")
    ...
    return (metrics_feats_selection_classifers, metrics_total, features_scores_total, topkFeatures_total,
            ...
            three_features_dict, all_the_best_y_test_pred_full, all_the_best_y_score_pred_full)
# folder_time1 = datetime.now().strftime("%Y-%m-%d_%I-%M-%S_%p")
            # plt.savefig(
            #     f'/media/linjianwei/T7_Shield/output/visual_select_times/{feats_selection_name}_{classifiers_name}_{folder_time1}.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_featsMat', type=str,
                        default='/media/linjianwei/data_16T/output/Paper_Pypathomics(example)/M_matrix_TCGA_171_with_label.csv',
                        help='Name of Experiment')
    parser.add_argument('--n_features', type=int,
                        default=6, help='number of top features')
    parser.add_argument('--kfold', type=int,
                        default=5, help='k fold cross-validation')
    parser.add_argument('--feature_score_method', type=str,
                        default='addone', help='feature score methods. one of addone or weighted')
    parser.add_argument('--corr_threshold', type=int,
                        default=0.9, help='remove the redundant features with correlation of 0.9')
    parser.add_argument('--n_repeats', type=int,
                        default=100, help='how many time to repeat the experiment')
    parser.add_argument('--list_feats_selection', type=list,
                        default=['Lasso', 'XGBoost', 'RandomForest', 'Elastic-Net', 'RFE', 'Univariate', 'mrmr', 'ttest', 'ranksums', 'mutualInfo'],
                        help="features selection methods. some of ['Univariate', 'mutualInfo','mrmr', \
                        'ttest', 'ranksums', 'rfe', 'chi'], Running speed are very slow: 'XGBoost', 'RFE'")
    parser.add_argument('--list_classifers', type=list,
                        default=['QDA', 'LDA', 'RandomForest', 'DecisionTree', 'KNeigh', 'LinearSVC', 'MLP', 'GaussianNB', 'SGD', 'SVC_rbf', 'AdaBoost'],
                        help="Classification methods. some of ['QDA', 'LDA', 'RandomForest', 'DecisionTree', \
                        'KNeigh', 'LinearSVC', 'MLP', 'GaussianNB', 'SGD', 'SVC_rbf', 'AdaBoost', 'GaussianProcess', 'Ridge']")
    parser.add_argument('--n_workers', type=int,
                        default=10,
                        help="parallel running, set a number lower than the core num of CPU ")
    args = parser.parse_args()

    for i in tqdm(range(1)):
        list_feats_selection = [list_feats_selection_Dict[args.list_feats_selection[i]] for i in
                                range(len(args.list_feats_selection))]
        list_classifers = [list_classifers_Dict[args.list_classifers[i]] for i in range(len(args.list_classifers))]

        df_features_labels = pd.read_csv(args.path_featsMat)
        df_labels = df_features_labels['label']
        df_patients_id = df_features_labels['patient_id']
        df_features = df_features_labels.drop(
            columns=['patient_id', 'patient_id_average_10', 'patient_id_median_10', 'patient_id_std_10',
                     'patient_id_kurtosis_10', 'patient_id_skew_10', 'patient_id_average_20', 'patient_id_median_20',
                     'patient_id_std_20', 'patient_id_kurtosis_20', 'patient_id_skew_20', 'label'])
        # df_features = df_features_labels.drop(columns=['patient_id', 'label'])
        features_name = df_features.columns.to_list()
        # features_name = df_features_labels.columns.to_list()

        #  remove the constant varibles
        df_total, selected_featsName_from_var_thresh = varianceThreshold_rm(X=df_features,
                                                                            var_thresh=0)  # varianceThreshold_rm(X, var_thresh=0.01)

        # remove redundant variables
        df_features_training_uncorr, dropped_featsName = remove_correlated_features(df_total,
                                                                                    corr_threshold=args.corr_threshold)
        selected_features_from_coorelated_feats = df_features_training_uncorr.columns.to_list()
        # df_features_training_uncorr = df_total
        ...
        df_features_training_uncorr_label_ptID = pd.concat([df_features_training_uncorr, df_labels, df_patients_id],
                                                           axis=1)

        # shuffle the DataFrame rows
        # df_features_training_uncorr_label_ptID = df_features_training_uncorr_label_ptID.sample(
        #     frac=1)  ## if reset the index    .reset_index(drop=True)
        df_features_training_uncorr_label_ptID = df_features_training_uncorr_label_ptID
        df_total_labels = df_features_training_uncorr_label_ptID[['label']]  #### Y  ground turth
        df_patients_id = df_features_training_uncorr_label_ptID[['patient_id']]
        df_features_training_uncorr = df_features_training_uncorr_label_ptID.drop(['patient_id', 'label'],
                                                                                  axis=1)  #### X

        zeros_num = df_total_labels[df_total_labels['label'] == 0]
        print('0: ', zeros_num.shape[0], '1: ',
              df_total_labels.shape[0] - zeros_num.shape[0])  #### .shape[0] return the num of row
        ...
        (metrics_feats_selection_classifers, metrics_total, features_scores_total, topkFeatures_total, best_params_list_total,
         metrics_feats_selection_classifers_fpr, metrics_feats_selection_classifers_tpr,
         metrics_feats_selection_classifers_fpr_fill_std, metrics_feats_selection_classifers_tpr_fill_std,
         three_features_dict, all_the_best_y_test_pred_full, all_the_best_y_score_pred_full) \
            = kfold_classifier_featureselection(list_feats_selection, list_classifers, args.kfold,
                                                df_features_training_uncorr, df_total_labels, df_patients_id,
                                                n_repeats=args.n_repeats, n_features=args.n_features,
                                                n_workers=args.n_workers,
                                                feature_score_method=args.feature_score_method)  ##addone
        ...
        df_acc_table = pd.DataFrame(data=metrics_feats_selection_classifers[:, :, 0],
                                    index=[str(feats_nm) for feats_nm in args.list_feats_selection],
                                    columns=[str(cla_nm) for cla_nm in args.list_classifers])
        ...
        # useful_data_TCGA = pd.read_excel('/media/linjianwei/data_16T/output/Paper_Pypathomics(example)/useful_data_TCGA.xlsx')
        # useful_data_TCGA = useful_data_TCGA[['ID', 'event_status', 'survival_time']]
        # all_the_best_y_test_pred_full['patient_id2'] = all_the_best_y_test_pred_full['patient_id'].str[:12]
        # all_the_best_y_test_pred_full_with_time = pd.merge(all_the_best_y_test_pred_full, useful_data_TCGA,
        #                                                    left_on='patient_id2', right_on='ID', how='left')
        # all_the_best_y_test_pred_full_with_time.to_csv('/home/linjianwei/PycharmProjects/Project_LXJ'
        #                                                '/visual_select_times/all_the_best_y_test_pred_full_with_time.csv', index=False)
        # all_the_best_y_score_pred_full['patient_id2'] = all_the_best_y_score_pred_full['patient_id'].str[:12]
        # all_the_best_y_score_pred_full_with_time = pd.merge(all_the_best_y_score_pred_full, useful_data_TCGA,
        #                                                    left_on='patient_id2', right_on='ID', how='left')
        # all_the_best_y_score_pred_full_with_time.to_csv('/home/linjianwei/PycharmProjects/Project_LXJ'
        #                                                 '/visual_select_times/all_the_best_y_score_pred_full_with_time.csv', index=False)


        folder_time = datetime.now().strftime("%Y-%m-%d_%I-%M-%S_%p")
        results_to_save_path = f'/home/linjianwei/PyPathomics_TCGA171_100times_5folds_cross_validation_2year' + folder_time
        if not os.path.exists(results_to_save_path):
            os.mkdir(results_to_save_path)

        # radar chart,5*11
        radar_chart_set(len(args.list_feats_selection), len(args.list_classifers), three_features_dict,
                        results_to_save_path)
        # ROC，5*11
        ROC_Chart_set(len(args.list_feats_selection), len(args.list_classifers), metrics_feats_selection_classifers_fpr,
        ...
        # 将所有的特征选择次数图表移出来保存到指定位置
        source_path = '/home/linjianwei/PycharmProjects/Project_LXJ/visual_select_times'
        destination_path = results_to_save_path
