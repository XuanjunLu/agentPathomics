"""
borrowed from pyradiomics
modified by Yuxin Wu
support roi level features
2024/02/18
wuusin@gmail.com
"""
...
class PathomicsFirstOrder(base.PathomicsFeaturesBase):
    r"""
  ...
  .. note::
    ...
    in PyPathomics, set ``voxelArrayShift`` to 0.
  """

    def __init__(self, inputImage, inputMask, **kwargs):
        super(PathomicsFirstOrder, self).__init__(inputImage, inputMask,
                                                  **kwargs)
        self.name = 'FirstOrder'
        self.pixelSpacing = inputImage.GetSpacing()
        self.ppm = kwargs.get('ppm', 0.25)
    ...
    def _initVoxelBasedCalculation(self):
        super(PathomicsFirstOrder, self)._initVoxelBasedCalculation()
        ...
        if self.masked:
            size = numpy.max(self.labelledVoxelCoordinates, 1) - numpy.min(
                self.labelledVoxelCoordinates, 1) + 1
        else:
        ...
        self.kernelOffsets = numpy.append(self.kernelOffsets, [[0, 0, 0]],
                                          axis=0)  # add center voxel
    ...
    def _initCalculation(self, voxelCoordinates=None):
        ...
        if voxelCoordinates is None:
            # print('mask array:', numpy.sum(self.maskArray))
            self.targetVoxelArray = self.imageArray[self.maskArray].astype(
                'float').reshape((1, -1))
            
            # rois = measure.label(self.maskArray)
            # self.targetROIArray = [self.imageArray[rois == i] for i in numpy.unique(rois) if i != 0]
            ...
            # print('p_i', p_i)
            p_is = []
            for i in np.unique(self.rois):
                if i ==0:
                    continue
                _, _p_i = numpy.unique(self.discretizedImageArray[self.rois == i], return_counts=True)
                # p_is.append(_p_i.reshape(1,-1).astype('float'))
                p_is.append(_p_i.reshape(1,-1))
        else:
            # voxelCoordinates shape (Nd, Nvox)
            voxelCoordinates = voxelCoordinates.copy() + self.settings.get(
                'kernelRadius', 1)  # adjust for padding
            kernelCoords = self.kernelOffsets[:,
                                              None, :] + voxelCoordinates[:, :,
                                                                          None]  # Shape (Nd, Nvox, Nk)
            kernelCoords = tuple(kernelCoords)  # shape (Nd, (Nvox, Nk))

            self.targetVoxelArray = self.imageArray[
                kernelCoords]  # shape (Nvox, Nk)

            p_i = numpy.empty(
                (voxelCoordinates.shape[1],
                 len(self.coefficients['grayLevels'])))  # shape (Nvox, Ng)
        ...
        for i in range(len(p_is)):
            sumBins = numpy.sum(p_is[i], 1, keepdims=True).astype('float')
            sumBins[sumBins == 0] = 1
            p_is[i] = p_is[i].astype('float') / sumBins
        self.coefficients['p_is'] = p_is
    ...
    """
        ...
        if moment == 1:
            ...
            mn = numpy.nanmean(a, keepdims=True)
            s = numpy.power((a - mn), moment)
            # return numpy.nanmean(s, 1)
            return numpy.nanmean(s)
    ...
    **1. Energy**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nansum(roi**2))
        # print(res)
    ...
    **2. Total Energy**
    ...
    .. note::
      In PyPathomics, use pixels per micron
    """
        ...
        return self.getEnergyFeatureValue() * self.ppm * self.ppm
    ...
    **3. Entropy**
    ...
    """
        ...
        p_is = self.coefficients['p_is']

        eps = numpy.spacing(1)
        # return -1.0 * numpy.sum(p_i * numpy.log2(p_i + eps), 1)
        res = []
        for i in range(len(p_is)):
            res.append(-1.0 * numpy.sum(p_is[i] * numpy.log2(p_is[i] + eps), 1))
        return numpy.array(res).astype('float')
    ...
    **5. 10th percentile**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanpercentile(roi, 10))
        return numpy.array(res).astype('float')
    ...
    **6. 90th percentile**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanpercentile(roi, 90))
        return numpy.array(res).astype('float')
    ...
    **9. Median**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanmedian(roi))
        return numpy.array(res).astype('float')
    ...
    **10. Interquartile Range**
    ...
    """
        ...
        # return numpy.nanpercentile(self.targetVoxelArray, 75,
        #                            1) - numpy.nanpercentile(
        #                                self.targetVoxelArray, 25, 1)
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanpercentile(roi, 75) - numpy.nanpercentile(roi, 25))
        return numpy.array(res).astype('float')
    ...
    **11. Range**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanmax(roi) - numpy.nanmin(roi))
        return numpy.array(res).astype('float')
    ...
    **12. Mean Absolute Deviation (MAD)**
    ...
    """
        ...
        res = []
        for roi in self.targetROIArray:
            res.append(numpy.nanmean(numpy.absolute(roi - numpy.nanmean(roi))))
        return numpy.array(res).astype('float')
    ...
    **13. Robust Mean Absolute Deviation (rMAD)**
    ...
    """
        ...
        #     numpy.absolute(percentileArray -
        #                    numpy.nanmean(percentileArray, 1, keepdims=True)),
        #     1)
        res = []
        for i in range(len(self.targetROIArray)):
            roi = self.targetROIArray[i]
            percentileArray = roi.copy()
            msk = ~numpy.isnan(percentileArray)
            msk[msk] = ((percentileArray - prcnt10[i])[msk] < 0) | (
                (percentileArray - prcnt90[i])[msk] > 0)
            percentileArray = percentileArray.astype('float')
            percentileArray[msk] = numpy.nan
            r = numpy.nanmean(
                numpy.absolute(percentileArray -
                               numpy.nanmean(percentileArray)),
                0)
            # print(r)
            res.append(r)
        return numpy.array(res).astype('float')
    ...
    **14. Root Mean Squared (RMS)**
    ...
    """
        ...
        shiftedParameterArray = self.targetVoxelArray + self.voxelArrayShift
        # Nvox = numpy.sum(~numpy.isnan(self.targetVoxelArray),
        #                  1).astype('float')
        # return numpy.sqrt(numpy.nansum(shiftedParameterArray**2, 1) / Nvox)
        res = []
        for roi in self.targetROIArray:
            Nvox = numpy.sum(~numpy.isnan(roi)).astype('float')
            res.append(numpy.sqrt(numpy.nansum(roi**2) / Nvox))
        return numpy.array(res).astype('float')
    ...
    **16. Skewness**
    ...
    """
        ...
        for roi in self.targetROIArray:
            ...
            # m2[m2 == 0] = 1
            # m3[m2 == 0] = 0
            # res.append(m3 / m2**1.5)
            res.append(scipy.stats.skew(roi))
        return numpy.array(res).astype('float')
    ...
    **17. Kurtosis**
    ...
    .. note::
      ...
      distributions. The PyPathomics kurtosis is not corrected, yielding a value 3 higher than the IBSI kurtosis.
    """
        ...
        for roi in self.targetROIArray:
            ...
            # m2[m2 == 0] = 1
            # m4[m2 == 0] = 0
            # res.append(m4 / m2**2.0)
            res.append(scipy.stats.kurtosis(roi,fisher=False))
        return numpy.array(res).astype('float')
    ...
    **19. Uniformity**
    ...
    """
        ...
        res = []
        for p_i in self.coefficients['p_is']:
            res.append(numpy.nansum(p_i**2, 1))
        return numpy.array(res).astype('float')
