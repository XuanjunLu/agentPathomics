...
class PathomicsNGTDM(base.PathomicsFeaturesBase):
  r"""
  ...
  def __init__(self, inputImage, inputMask, **kwargs):
    super(PathomicsNGTDM, self).__init__(inputImage, inputMask, **kwargs)
    self.name = 'NGTDM'
    self.P_ngtdm = None
  ...
  def _calculateCoefficients(self):
    ...
    for _i in range(len(self.P_ngtdm)):
      # No of voxels that have a valid region, lesser equal to Np
      Nvp = numpy.sum(self.P_ngtdm[_i][:, :, 0], 1)  # shape (Nvox,)
      p_i = self.P_ngtdm[_i][:, :, 0] / Nvp[:, None]
      s_i = self.P_ngtdm[_i][:, :, 1]
      ivector = self.P_ngtdm[_i][:, :, 2]
      Ngp = numpy.sum(self.P_ngtdm[_i][:, :, 0] > 0, 1)
      p_zero = numpy.where(p_i == 0)
    ...
    self.coefficients['Nvp'] = Nvps  # shape (Nv,)

    # Normalize P_ngtdm[:, 0] (= n_i) to obtain p_i
    self.coefficients['p_i'] = p_is

    self.coefficients['s_i'] = s_is
    self.coefficients['ivector'] = ivectors

    # Ngp = number of graylevels, for which p_i > 0
    self.coefficients['Ngp'] = Ngps

    self.coefficients['p_zero'] = p_zeros
  ...
  def getCoarsenessFeatureValue(self):
    ...
    res = []
    for _i in range(len(self.P_ngtdm)):
      sum_coarse = numpy.sum(p_i[_i] * s_i[_i], 1)
      sum_coarse[sum_coarse != 0] = 1 / sum_coarse[sum_coarse != 0]
      sum_coarse[sum_coarse == 0] = 1e6
      res.append(sum_coarse)
  ...
  def getContrastFeatureValue(self):
    ...
    for _i in range(len(self.P_ngtdm)):
      div = Ngp[_i] * (Ngp[_i] - 1)
      contrast = (numpy.sum(p_i[_i][:, :, None] * p_i[_i][:, None, :] * (i[_i][:, :, None] - i[_i][:, None, :]) ** 2, (1, 2)) *
                  numpy.sum(s_i[_i], 1) / Nvp[_i])
  ...
  def getBusynessFeatureValue(self):
    ...
    for _i in range(len(self.P_ngtdm)):
      ...
      # Remove terms from the sum where p_i = 0 or p_j = 0
      absdiff[p_zero[_i][0], :, p_zero[_i][1]] = 0
      absdiff[p_zero[_i][0], p_zero[_i], :] = 0

      absdiff = numpy.sum(absdiff, (1, 2))

      busyness = numpy.sum(p_i[_i] * s_i[_i], 1)
      busyness[absdiff != 0] = busyness[absdiff != 0] / absdiff[absdiff != 0]
      busyness[absdiff == 0] = 0
      res.append(busyness)
    return numpy.array(res).astype('float')
  ...
  def getComplexityFeatureValue(self):
    ...
    for _i in range(len(self.P_ngtdm)):
      ...
      # Remove terms from the sum where p_i = 0 or p_j = 0
      numerator[p_zero[_i][0], :, p_zero[_i][1]] = 0
      numerator[p_zero[_i][0], p_zero[_i][1], :] = 0

      divisor = p_i[_i][:, :, None] + p_i[_i][:, None, :]
      divisor[divisor == 0] = 1

      complexity = numpy.sum(numpy.abs(i[_i][:, :, None] - i[_i][:, None, :]) * numerator / divisor, (1, 2)) / Nvp[_i]
      res.append(complexity)
    return numpy.array(res).astype('float')
  ...
  def getStrengthFeatureValue(self):
    ...
    res = []
    for _i in range(len(self.P_ngtdm)):
      sum_s_i = numpy.sum(s_i[_i], 1)
      strength = (p_i[_i][:, :, None] + p_i[_i][:, None, :]) * (i[_i][:, :, None] - i[_i][:, None, :]) ** 2

      # Remove terms from the sum where p_i = 0 or p_j = 0
      strength[p_zero[_i][0], :, p_zero[_i][1]] = 0
      strength[p_zero[_i][0], p_zero[_i][1], :] = 0
